...
export default function ImagesPage() {
  ...
  return (
    <PageShell title="Images" subtitle="Text-to-image on free cloud models.">
      <div className="mb-3 flex flex-col gap-2 rounded-2xl border border-line bg-surface-2 p-2 sm:flex-row">
        <input
          value={prompt}
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {STARTERS.map((s) => (
          <button
            ...
            onClick={() => generate(s)}
            disabled={busy}
            className="rounded-full border border-line bg-surface-2 px-3 py-1.5 text-xs text-muted transition-colors hover:text-ink disabled:opacity-40"
          >
            {s.split(",")[0]}
          </button>
        ))}
      ...
      {items.length === 0 ? (
        <div className="grid place-items-center rounded-2xl border border-dashed border-line py-20 text-center">
          <p className="text-sm text-faint">
            ...
            or set <code className="font-mono text-muted">TOGETHER_API_KEY</code> /{" "}
            <code className="font-mono text-muted">HF_TOKEN</code> for FLUX.
          </p>
        </div>
      ) : (
          ...
          <AnimatePresence>
            {items.map((it) => (
              <motion.div
                ...
                className="group relative aspect-square overflow-hidden rounded-2xl border border-line bg-surface-3"
              >
                {it.status === "loading" && (
                ...
                <div className="absolute inset-0 flex items-end bg-gradient-to-t from-black/80 to-transparent p-3 opacity-0 transition-opacity group-hover:opacity-100">
                  <div>
                    <p className="line-clamp-2 text-xs text-ink">{it.prompt}</p>
                    {it.provider && (
