...
export type AgentOutcome =
  | { ok: true; text: string; sources: { title: string; url: string }[]; model: string; backend: "tavily" | "openrouter" }
...
export async function runWebAgent(input: {
  ...
  if (tav.hasKey) {
    ...
    if (!s.results.length) return { ok: true, text: "", sources: [], model: "none", backend: "tavily" };
    ...
    return { ok: true, text: out.text, sources, model: out.model, backend: "tavily" };
  }
