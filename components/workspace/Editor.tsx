...
export default function Editor() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface">
      {/* tabs */}
      <div className="flex flex-none items-center overflow-x-auto border-b border-line bg-surface-2">
        {tabs.length === 0 && (
          <span className="px-4 py-2.5 text-xs text-faint">No open files</span>
        )}
        {tabs.map((path) => {
          const f = vfs[path];
          ...
          return (
            <button
              ...
              className={`group flex flex-none items-center gap-2 border-r border-line px-3 py-2.5 text-xs ${
                isActive
                  ? "bg-surface text-ink"
            ...
            >
              ...
              <span
                ...
                className="ml-1 rounded px-1 text-faint opacity-0 hover:bg-surface-3 hover:text-ink group-hover:opacity-100"
              >
                ×
              </span>
            </button>
      ...
      {!file ? (
        ...
        <div className="relative flex min-h-0 flex-1">
          ...
          <div
            ref={gutterRef}
            className="select-none overflow-hidden border-r border-line bg-surface-2 px-3 py-3 text-right font-mono text-xs leading-[1.6] text-faint"
          >
      ...
      {/* status bar */}
      <div className="flex flex-none items-center justify-between border-t border-line bg-surface-2 px-3 py-1.5 font-mono text-[11px] text-faint">
