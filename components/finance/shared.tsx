    ...
    <div>
      <label
        ...
        className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed px-6 py-10 text-center transition-colors ${
          over ? "border-navy/50 bg-navy/[0.04]" : "border-line hover:border-black/30 hover:bg-surface-2/60"
        }`}
      >
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" className="text-faint" aria-hidden="true">
          <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12" />
        </svg>
        <span className="text-sm font-medium text-ink">{hint}</span>
        <span className="font-mono text-[11px] text-faint">.csv — parsed in your browser</span>
        <input
          type="file"
          accept=".csv,text/csv"
          hidden
    ...
    <div className="space-y-2">
      {items.map((it, i) => (
        <div key={i} className="flex items-center gap-3">
          <span className="w-40 flex-none truncate text-sm text-muted" title={it.label}>
            {it.label}
          </span>
...
export function PrivacyNote() {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-emerald-600/20 bg-emerald-600/[0.05] px-3 py-2 text-xs text-success">
...
export function Panel({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-2xl border border-line bg-surface-2/60 p-5">{children}</div>
  );
}
