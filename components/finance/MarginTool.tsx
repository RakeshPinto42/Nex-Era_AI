        ...
        <>
          ...
          <div className="overflow-hidden rounded-2xl border border-line">
            <div className="flex items-center justify-between border-b border-line bg-surface-2/60 px-4 py-2.5">
              <span className="font-mono text-[11px] uppercase tracking-widest text-faint">Segment detail</span>
                ...
                <tbody>
                  {result.rows.map((r, i) => (
                    <tr key={i} className="border-b border-line last:border-0">
                      ...
                      <td className="px-4 py-2 text-right font-mono text-muted">{fmtMoney(r.cost)}</td>
                      <td className="px-4 py-2 text-right font-mono text-ink">{fmtMoney(r.profit)}</td>
                      <td className={`px-4 py-2 text-right font-mono ${r.margin >= 25 ? "text-emerald-600" : "text-danger"}`}>
                        {r.margin.toFixed(1)}%
                      </td>
                    </tr>
                  ))}
